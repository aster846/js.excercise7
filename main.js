...
function searchStudent() {
    ...
    productRows.forEach(row => {
        let products = Array.from(row.querySelectorAll(".product-card"));
        let matched = [];
        let notMatched = [];
        products.forEach(product => {
            ...
            if (title.includes(keyword)) {
                ...
                product.style.display = keyword === "" ? "block" : "none";
                notMatched.push(product);
            }
        });
        row.innerHTML = "";
        matched.forEach(p => row.appendChild(p));
        notMatched.forEach(p => row.appendChild(p));
    });
}
...
function addToCart(bookName, price, bookImg) { 
    ...
    if (existingItem) {
        ...
        // CHANGE 2: Added 'image: bookImg' to the saved object
...
// === 7. DYNAMIC PRODUCT DETAIL POPUP (FIXED) ===
    ...
    if (card && !isButton) {
        ...
        if (modalAddToCartBtn) {
            modalAddToCartBtn.onclick = function() {
                // CHANGE 3: Pass the 'img' variable into the function
